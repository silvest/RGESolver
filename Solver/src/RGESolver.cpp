...
RGESolver::RGESolver() : yuR(3, 0.), yuI(3, 0.), ydR(3, 0.), ydI(3, 0.), yeR(3, 0.), yeI(3, 0.), dyuR(3, 0.), dyuI(3, 0.), dydR(3, 0.), dydI(3, 0.), dyeR(3, 0.), dyeI(3, 0.)
{
  ...
  //-----SM SCALAR PARAMETERS I/O----------
  {
    ...
    Operators0F["dg2"] = &dg2;
    Operators0F["dg3"] = &dg3;
    Operators0F["dmh2"] = &dmh2;
    Operators0F["dlambda"] = &dlambda;
  ...
  Setter2F["dYuR"] = boost::bind(&Yukawa_set, &dyuR, _1, _2, _3);
  Setter2F["dYuI"] = boost::bind(&Yukawa_set, &dyuI, _1, _2, _3);
  Getter2F["dYuR"] = boost::bind(&Yukawa, &dyuR, _1, _2);
  Getter2F["dYuI"] = boost::bind(&Yukawa, &dyuI, _1, _2);

  Setter2F["dYeR"] = boost::bind(&Yukawa_set, &dyeR, _1, _2, _3);
  Setter2F["dYeI"] = boost::bind(&Yukawa_set, &dyeI, _1, _2, _3);
  Getter2F["dYeR"] = boost::bind(&Yukawa, &dyeR, _1, _2);
  Getter2F["dYeI"] = boost::bind(&Yukawa, &dyeI, _1, _2);

  Setter2F["dYdR"] = boost::bind(&Yukawa_set, &dydR, _1, _2, _3);
  Setter2F["dYdI"] = boost::bind(&Yukawa_set, &dydI, _1, _2, _3);
  Getter2F["dYdR"] = boost::bind(&Yukawa, &dydR, _1, _2);
  Getter2F["dYdI"] = boost::bind(&Yukawa, &dydI, _1, _2);
  {
...
void RGESolver::EvolveSMEFTOnly(double muI, double muF)
{
  ...
  if (!cache || muIcache != muI || muFcache != muF)
  {
    ...
    else
    {
      ...
      muFcache = muF;
      double SMcache[59]={0.};
      // Save into xcache the initial conditions
      for (int i = 0; i < NX; i++)
      {
        xcache[i] = x[i];
      }
      // Evolve numerically from muI to muF with one coefficient at a time to determine one column of the evolution matrix
      ...
      for (int i = 59; i < NX; i++)
      {
        ...
        //Neglect the SMEFT contributions to the evolution of the SM couplings
        SMEFTinSMbeta = false;       
        ...
        for (int j = 59; j < NX; j++) 
        {
          SMEFTevcache[idx(j, i)] = x[j] * muI * muI;
        }
      }
      ...
      // Restore the SMEFT contributions to the evolution of the SM couplings
      SMEFTinSMbeta = true;
    }
  }
  // Perform the evolution using the cache
  // Save into xcache the initial conditions for the SM parameters
  for (int i = 0; i < 59; i++)
  {
    xcache[i] = x[i];
  }
...
{
  if ((i <= 2) && (i >= 0) && (j <= 2) && (j >= 0) && (k <= 2) && (k >= 0) && (l <= 2) && (l >= 0))
  {
